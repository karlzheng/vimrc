...
fun! s:MarksBrowser_Window_Close()
    " Make sure the MarksBrowser window exists
	let l:winnum = bufwinnr(s:win_title)
	"let l:winnum = bufwinnr('"'.s:win_title.'"')
    if l:winnum == -1
        return
    endif
    if winnr() == l:winnum
        " Already in the MarksBrowser window. Close it and return
        if winbufnr(2) != -1
        ...
        let l:winnum = bufwinnr(curbufnr)
        if winnr() != l:winnum
            exe l:winnum . 'wincmd w'
        endif
    endif
...
fun! s:ToggleMarksBrowser()
	for i in range(1, bufnr("$"))                                        
		...
		if l:wn != -1
			if matchstr(l:buffilename, s:win_title) != ""
				"echo "match".i
			endif
		endif
	endfor
	...
	if l:winnum != -1 
		if (bufname(winbufnr(l:winnum)) ==? s:win_title)
			call s:MarksBrowser_Window_Close()
			return
	    endif
	endif
...
fun! s:ShowMarksWin(winNo)
  if winnr() != a:winNo
    ...
    if a:winNo != -1
      ...
      exec "bel 40 vs " . escape(s:win_title, ' ')
      let s:bufNo = bufnr('%')
    endif
    call s:setupBindings()
    let s:isShown = 1
    call s:Fill(lines, lnum)
	"add by karlzheng begin
	exec "wincmd h"
	"add by karlzheng end
  else
    close
  endif
endf
